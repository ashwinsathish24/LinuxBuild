...
QtObject {
    ...
    function connectToNetwork(network, session, onPasswordNeeded): void {
        ...
        if (network.isSecure) {
            ...
            if (hasSavedProfile) {
                Nmcli.connectToNetwork(network.ssid, "", network.bssid, handlePassword);
            } else {
                Nmcli.connectToNetworkWithPasswordCheck(network.ssid, network.isSecure, handlePassword, network.bssid);
            }
        } else {
